...
frisby.create('It should find a team by its key')
  .get('http://localhost:8080/api/teams?key=cacapava-do-sul')
  .expectJSON([{"name": team, "enabled": true, "id": 1, "key": "cacapava-do-sul"}])
  .expectStatus(200)
  .toss()

frisby.create('It returns no content when it tries to find an invalid key')
  .get('http://localhost:8080/api/teams?key=foo')
  .expectStatus(204)
  .toss()

frisby.create('It should not accept a delete request on teams endpoint')
  .delete('http://localhost:8080/api/teams')
  .expectStatus(405)
  .toss()

frisby.create('It should not accept a put request on teams endpoint')
